...
- **8 builtin polyhedra** (cube, tetrahedron, octahedron, dodecahedron, icosahedron, triakis tetrahedron, snub cube, noperthedron). All carry exact algebraic vertex tables (`ExactVec3`) for interval-arithmetic verification.
- **8 baseline solvers**: `random_quat`, `face_normal_pairs`, `nelder_mead`, `random_then_refine`, `hopf_grid`, `imperts` (port of Tom 7's SIGBOVIK 2025 refiner), `gosain_grimmer` (port of arXiv:2509.08190 subgradient), `patch_aware` (Tom 7's patch decomposition; v0.2.0 with reconnaissance-first scan).
- **Two certification tiers**: `F64Epsilon` (f64 self-consistency) and `IntervalSnap` (rigorous lower-bound via `inari` interval arithmetic over the algebraic vertex tables). The bench runner attempts `IntervalSnap` first, falls back to `F64Epsilon`.
...
- [`docs/verification.md`](docs/verification.md) — F64Epsilon + IntervalSnap pipeline; the noperthedron soundness gate.
